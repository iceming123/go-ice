...
import (
	...
	//"github.com/iceming123/go-ice/ethdb"
...
type Database struct {
	diskdb icedb.Database // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb icedb.Database) *Database {
...
func NewDatabaseWithCache(diskdb icedb.Database, cache int) *Database {
...
func (db *Database) Cap(limit common.StorageSize) error {
	...
	if flushPreimages {
		for hash, preimage := range db.preimages {
			...
			if batch.ValueSize() > icedb.IdealBatchSize {
...
func (db *Database) commit(hash common.Hash, batch icedb.Batch) error {
