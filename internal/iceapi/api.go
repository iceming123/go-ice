...
var (
	LocalTxMetrics = metrics.NewRegisteredMeter("ice/prop/local_tx/in", nil)
)

// PublicICEAPI provides an API to access True related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicICEAPI struct {
	b Backend
}

// NewPublicICEAPI creates a new True protocol API.
func NewPublicICEAPI(b Backend) *PublicICEAPI {
	return &PublicICEAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicICEAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
	price, err := s.b.SuggestPrice(ctx)
	return (*hexutil.Big)(price), err
}

// ProtocolVersion returns the current True protocol version this node supports
func (s *PublicICEAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicICEAPI) Syncing() (interface{}, error) {
...
//
// https://github.com/iceming123/go-ice/wiki/Management-APIs#personal_sign
...
// Note, this function is compatible with ice__sign and personal_sign. As such it recovers
// the address of:
...
//
// https://github.com/iceming123/go-ice/wiki/Management-APIs#personal_ecRecover
...
func RPCMarshalBlock(b *types.Block, inclTx bool, fullTx bool) (map[string]interface{}, error) {
	...
	formatMembers := func(commit *types.CommitteeMember) (map[string]interface{}, error) {
		members := map[string]interface{}{
			"coinbase":      commit.Coinbase.String(),
			"committeeBase": commit.CommitteeBase.String(),
	...
	if inclFruit {
		formatFruit := func(fruit *types.SnailBlock) (interface{}, error) {
			return map[string]interface{}{
				...
				"miner":      fruit.Header().Coinbase.String(),
...
func (s *PublicBlockChainAPI) GetChainRewardContent(blockNr rpc.BlockNumber, addr common.Address) map[string]interface{} {
	...
	if bytes.Equal(addr.Bytes(), empty.Bytes()) {
		...
		fields := map[string]interface{}{
			"Number":        hexutil.Uint64(blockNr),
			"time":          hexutil.Uint64(content.St),
			"stakingReward": types.FetchOneToWei(content.CommitteeBase, addr),
		}
		return fields
	}
}
...
func (s *PublicTransactionPoolAPI) sendIceRawTransaction(ctx context.Context, encodedTx hexutil.Bytes) (common.Hash, error) {
	tx := new(types.Transaction)
	if err := rlp.DecodeBytes(encodedTx, tx); err != nil {
		log.Error("api method sendIceRawTransaction error", "error", err)
		return common.Hash{}, err
	}
	//log.Info("api method sendIceRawTransaction info", "tx.info", tx.Info())
	return submitTransaction(ctx, s.b, tx)
}
...
func (s *PublicTransactionPoolAPI2) sendIceRawTransaction(ctx context.Context, encodedTx hexutil.Bytes) (common.Hash, error) {
	tx := new(types.Transaction)
	if err := rlp.DecodeBytes(encodedTx, tx); err != nil {
		log.Error("api method sendIceRawTransaction error", "error", err)
		return common.Hash{}, err
