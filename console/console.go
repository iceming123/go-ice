...
import (
	...
	"github.com/iceming123/go-ice/internal/jsre"
	"github.com/iceming123/go-ice/internal/web3ext"
...
func (c *Console) init(preload []string) error {
	...
	flatten := "var ice = web3.ice; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = ice.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines ice.getBalance(ice.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gice instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Gice metadata
	fmt.Fprintf(c.printer, "Welcome to the Gice JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + ice.coinbase);
		console.log("at block: " + ice.blockNumber + " (" + new Date(1000 * ice.getBlock(ice.blockNumber).timestamp) + ")");
