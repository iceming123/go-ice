...
import (
	...
	"github.com/iceming123/go-ice/accounts/abi/bind"
...
type SimulatedBackend struct {
	database   icedb.Database   // In memory database to store our testing data
...
func NewSimulatedBackendWithDatabase(database icedb.Database, alloc types.GenesisAlloc, gasLimit uint64) *SimulatedBackend {
...
func NewSimulatedBackend(alloc types.GenesisAlloc, gasLimit uint64) *SimulatedBackend {
	return NewSimulatedBackendWithDatabase(icedb.NewMemDatabase(), alloc, gasLimit)
}
...
func (b *SimulatedBackend) CallContract(ctx context.Context, call icechain.CallMsg, blockNumber *big.Int) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) PendingCallContract(ctx context.Context, call icechain.CallMsg) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) EstimateGas(ctx context.Context, call icechain.CallMsg) (uint64, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) callContract(ctx context.Context, call icechain.CallMsg, block *types.Block, statedb *state.StateDB) (*core.ExecutionResult, error) {
...
func (b *SimulatedBackend) FilterLogs(ctx context.Context, query icechain.FilterQuery) ([]types.Log, error) {
...
func (b *SimulatedBackend) SubscribeFilterLogs(ctx context.Context, query icechain.FilterQuery, ch chan<- types.Log) (icechain.Subscription, error) {
...
func (b *SimulatedBackend) SubscribeNewHead(ctx context.Context, ch chan<- *types.Header) (icechain.Subscription, error) {
...
func (fb *filterBackend) ChainDb() icedb.Database  { return fb.db }
