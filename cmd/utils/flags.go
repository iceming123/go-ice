...
// Package utils contains internal helper functions for ice commands.
package utils
...
import (
	...
	"github.com/iceming123/go-ice/accounts/keystore"
	"github.com/iceming123/go-ice/common"
	"github.com/iceming123/go-ice/common/fdlimit"
	"github.com/iceming123/go-ice/consensus"

	//"github.com/iceming123/go-ice/consensus/clique"
	"bytes"

	"github.com/iceming123/go-ice/consensus/minerva"
	"github.com/iceming123/go-ice/core"
	"github.com/iceming123/go-ice/core/snailchain"
	...
	"github.com/iceming123/go-ice/icestats"
	"github.com/iceming123/go-ice/les"
	"github.com/iceming123/go-ice/log"
	"github.com/iceming123/go-ice/metrics"
	"github.com/iceming123/go-ice/metrics/influxdb"
	"github.com/iceming123/go-ice/node"
	"github.com/iceming123/go-ice/p2p"
	"github.com/iceming123/go-ice/p2p/enode"
	"github.com/iceming123/go-ice/p2p/nat"
	"github.com/iceming123/go-ice/p2p/netutil"
...
var (
	...
	NetworkIdFlag = cli.Uint64Flag{
		...
		Value: ice.DefaultConfig.NetworkId,
	}
	...
	BFTPortFlag = cli.Uint64Flag{
		...
		Value: uint64(ice.DefaultConfig.Port),
	}
	BFTStandbyPortFlag = cli.Uint64Flag{
		Name:  "bftport2",
		Usage: "committee node standby port ",
		Value: uint64(ice.DefaultConfig.StandbyPort),
	}
	...
	defaultSyncMode = ice.DefaultConfig.SyncMode
	...
	LightPeersFlag = cli.IntFlag{
		...
		Value: ice.DefaultConfig.LightPeers,
	}
	...
	TxPoolPriceLimitFlag = cli.Uint64Flag{
		...
		Value: ice.DefaultConfig.TxPool.PriceLimit,
	}
	TxPoolPriceBumpFlag = cli.Uint64Flag{
		Name:  "txpool.pricebump",
		Usage: "Price bump percentage to replace an already existing transaction",
		Value: ice.DefaultConfig.TxPool.PriceBump,
	}
	TxPoolAccountSlotsFlag = cli.Uint64Flag{
		Name:  "txpool.accountslots",
		Usage: "Minimum number of executable transaction slots guaranteed per account",
		Value: ice.DefaultConfig.TxPool.AccountSlots,
	}
	TxPoolGlobalSlotsFlag = cli.Uint64Flag{
		Name:  "txpool.globalslots",
		Usage: "Maximum number of executable transaction slots for all accounts",
		Value: ice.DefaultConfig.TxPool.GlobalSlots,
	}
	TxPoolAccountQueueFlag = cli.Uint64Flag{
		Name:  "txpool.accountqueue",
		Usage: "Maximum number of non-executable transaction slots permitted per account",
		Value: ice.DefaultConfig.TxPool.AccountQueue,
	}
	TxPoolGlobalQueueFlag = cli.Uint64Flag{
		Name:  "txpool.globalqueue",
		Usage: "Maximum number of non-executable transaction slots for all accounts",
		Value: ice.DefaultConfig.TxPool.GlobalQueue,
	}
	TxPoolLifetimeFlag = cli.DurationFlag{
		Name:  "txpool.lifetime",
		Usage: "Maximum amount of time non-executable transaction are queued",
		Value: ice.DefaultConfig.TxPool.Lifetime,
	}
	//fruit pool settings
	...
	GasTargetFlag = cli.Uint64Flag{
		...
		Value: ice.DefaultConfig.MinerGasFloor,
	}

	GasLimitFlag = cli.Uint64Flag{
		Name:  "gaslimit",
		Usage: "Target gas ceiling for fast block",
		Value: ice.DefaultConfig.MinerGasCeil,
	}
	...
	GasPriceFlag = BigFlag{
		...
		Value: ice.DefaultConfig.GasPrice,
	}
	...
	IcestatsURLFlag = cli.StringFlag{
		Name:  "icestats",
		Usage: "Reporting URL of a icestats service (nodename:secret@host:port)",
	}
	...
	GpoBlocksFlag = cli.IntFlag{
		...
		Value: ice.DefaultConfig.GPO.Blocks,
	}
	GpoPercentileFlag = cli.IntFlag{
		Name:  "gpopercentile",
		Usage: "Suggested gas price is the given percentile of a set of recent transaction gas prices",
		Value: ice.DefaultConfig.GPO.Percentile,
	}
...
func setBftCommitteeKey(ctx *cli.Context, cfg *ice.Config) {
	var (
...
// for Gice and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses!(can search via `gice account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func setEtherbase(ctx *cli.Context, ks *keystore.KeyStore, cfg *ice.Config) {
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	log.Info("Maximum peer count", "ICE", ethPeers, "LES", lightPeers, "total", cfg.MaxPeers)
...
func setEthash(ctx *cli.Context, cfg *ice.Config) {
...
// SetIcechainConfig applies ice-related command line flags to the config.
func SetIcechainConfig(ctx *cli.Context, stack *node.Node, cfg *ice.Config) {
...
// RegisterIceService adds an Icechain client to the stack.
func RegisterIceService(stack *node.Node, cfg *ice.Config) {
	var err error
	...
	if err != nil {
		Fatalf("Failed to register the Icechain service: %v", err)
	}
}

// RegisterIceStatsService configures the Icechain Stats daemon and adds it to
// th egiven node.
func RegisterIceStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		// Retrieve both ice and les services
		var serv0 *ice.Icechain
		ctx.Service(&serv0)

		var lesServ *les.LightICE
		ctx.Service(&lesServ)

		return icestats.New(url, serv0, lesServ)
	}); err != nil {
		Fatalf("Failed to register the Icechain Stats service: %v", err)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gice.", map[string]string{
				"host": hosttag,
			})
		}
	}
...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node) icedb.Database {
	var (
...
func MakeChain(ctx *cli.Context, stack *node.Node) (fchain *core.BlockChain, schain *snailchain.SnailBlockChain, chainDb icedb.Database) {
	var err error
	...
	if !ctx.GlobalBool(FakePoWFlag.Name) {
		engine = minerva.New(minerva.Config{
			CacheDir:       stack.ResolvePath(ice.DefaultConfig.MinervaHash.CacheDir),
			CachesInMem:    ice.DefaultConfig.MinervaHash.CachesInMem,
			CachesOnDisk:   ice.DefaultConfig.MinervaHash.CachesOnDisk,
			DatasetDir:     stack.ResolvePath(ice.DefaultConfig.MinervaHash.DatasetDir),
			DatasetsInMem:  ice.DefaultConfig.MinervaHash.DatasetsInMem,
			DatasetsOnDisk: ice.DefaultConfig.MinervaHash.DatasetsOnDisk,
		})
	}
	if gcmode := ctx.GlobalString(GCModeFlag.Name); gcmode != "full" && gcmode != "archive" {
		Fatalf("--%s must be either 'full' or 'archive'", GCModeFlag.Name)
	...
	cache := &core.CacheConfig{
		...
		TrieNodeLimit: ice.DefaultConfig.TrieCache,
		TrieTimeLimit: ice.DefaultConfig.TrieTimeout,
	}
...
// new format.
//
// e.g. gice account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// gice --keystore /tmp/mykeystore --lightkdf account new
//
