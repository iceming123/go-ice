...
func sendContractTransaction(client *iceclient.Client, from, toAddress common.Address, value *big.Int, privateKey *ecdsa.PrivateKey, input []byte) common.Hash {
...
func queryTx(conn *iceclient.Client, txHash common.Hash, contract bool, pending bool, delegate bool) {
...
func PrintBalance(conn *iceclient.Client, from common.Address) {
	...
	fmt.Println("Your wallet valid balance is ", trueValue, "'true ", " lock balance is ", types.ToCoin(sbalance), "'true ")
}
...
func queryRewardInfo(conn *iceclient.Client, number uint64, start bool) {
...
func queryStakingInfo(conn *iceclient.Client, query bool, delegate bool) {
	header, err := conn.HeaderByNumber(context.Background(), nil)
	if err != nil {
		log.Fatal(err)
	}
