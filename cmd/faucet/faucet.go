...
import (
	...
	"github.com/iceming123/go-ice/iceclient"
...
var (
	...
	portFlag = flag.Int("port", 30313, "Listener port for the devp2p connection")
	bootFlag     = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag      = flag.Uint64("network", 0, "Network ID to use for the ICECHAIN protocol")
	statsFlag    = flag.String("icestats", "url", "icestats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of baseCoin to pay out per user request")
...
var (
	baseCoin = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
var (
...
type request struct {
	...
	Account common.Address     `json:"account"` // ICECHAIN address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an ICECHAIN light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // ICECHAIN protocol stack
	client *iceclient.Client   // Client connection to the Ethereum chain
...
func main() {
	...
	faucet, err := newFaucet(genesis, *portFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
func newFaucet(genesis *core.Genesis, port int, enodes []*enode.Node, network uint64, stats string,
	...
	// Assemble the ICECHAIN light client protocol
	...
	// Assemble the icestats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var lesServ *les.LightICE
			ctx.Service(&lesServ)

			return icestats.New(stats, nil, lesServ)
		}); err != nil {
			return nil, errors.New(fmt.Sprintf("Failed to register the Icechain Stats service: %v", err))
		}
	}
	...
	client := iceclient.NewClient(api)
...
// close terminates the ICECHAIN connection and tears down the faucet.
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance := new(big.Int).Div(f.balance, baseCoin)
...
// apiHandler handles requests for baseCoin grants and transaction statuses.
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err = send(wsconn, map[string]interface{}{
		"funds":    new(big.Int).Div(balance, baseCoin),
		"funded":   nonce,
		"peers":    f.stack.Server().PeerCount(),
		"requests": reqs,
	...
	for {
		...
		if timeout = f.timeouts[id]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), baseCoin)
...
// the uniqueness identifier (user id/username), username, avatar URL and ICECHAIN address to fund on success.
func authTwitter(url string, tokenV1, tokenV2 string) (string, string, string, common.Address, error) {
	...
	// and scrape it for the ICECHAIN address and profile URL. We need to load
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", "", common.Address{}, errors.New("No ICECHAIN address found to fund")
	}
	var avatar string
...
// API, returning the user id, username, avatar URL and ICECHAIN address to fund on
// success.
...
// returning the username, avatar URL and ICEChain address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the ICEChain address and profile URL.
	//
...
// authNoAuth tries to interpret a faucet request as a plain ICEChain address,
