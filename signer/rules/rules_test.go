...
import (
	...
	"github.com/iceming123/go-ice/signer/core"
	"github.com/iceming123/go-ice/signer/storage"
)
...
func (alwaysDenyUI) OnApprovedTx(tx iceapi.SignTransactionResult) {
	panic("implement me")
}
...
func (d *dummyUI) OnApprovedTx(tx iceapi.SignTransactionResult) {
...
func TestForwarding(t *testing.T) {
	...
	r.OnApprovedTx(iceapi.SignTransactionResult{})
...
func TestLimitWindow(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		response := iceapi.SignTransactionResult{
			Tx:  dummySigned(v),
...
func (d *dontCallMe) OnApprovedTx(tx iceapi.SignTransactionResult) {
	d.t.Fatalf("Did not expect next-handler to be called")
}
