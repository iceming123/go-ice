...
import (
	...
	"github.com/iceming123/go-ice/accounts/usbwallet"
...
type ExternalAPI interface {
	...
	SignTransaction(ctx context.Context, args SendTxArgs, methodSelector *string) (*iceapi.SignTransactionResult, error)
...
type SignerUI interface {
	...
	OnApprovedTx(tx iceapi.SignTransactionResult)
...
func (api *SignerAPI) SignTransaction(ctx context.Context, args SendTxArgs, methodSelector *string) (*iceapi.SignTransactionResult, error) {
	var (
		err    error
	...
	response := iceapi.SignTransactionResult{Raw: rlpdata, Tx: signedTx}
...
// Note, this function is compatible with ice_sign and personal_sign. As such it recovers
// the address of:
...
func (api *SignerAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	...
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid Icechain signature (V is not 27 or 28)")
	}
...
func SignHash(data []byte) ([]byte, string) {
	msg := fmt.Sprintf("\x19Icechain Signed Message:\n%d%s", len(data), data)
