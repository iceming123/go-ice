...
import (
	...
	"github.com/iceming123/go-ice/common/bitutil"
...
func GetChtRoot(db icedb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
// Note that sectionIdx is specified according to LES/1 CHT section size.
func StoreChtRoot(db icedb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
type ChtIndexerBackend struct {
	diskdb, trieTable    icedb.Database
	odr                  OdrBackend
...
func NewChtIndexer(db icedb.Database, odr OdrBackend, size, confirms uint64) *core.ChainIndexer {
	trieTable := icedb.NewTable(db, ChtTablePrefix)
	...
	return core.NewChainIndexer(db, icedb.NewTable(db, "chtIndex-"), backend, size, confirms, time.Millisecond*100, "cht", true)
}
...
func GetBloomTrieRoot(db icedb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db icedb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewBloomTrieIndexer(db icedb.Database, odr OdrBackend, parentSize, size uint64) *core.ChainIndexer {
	trieTable := icedb.NewTable(db, BloomTrieTablePrefix)
	...
	return core.NewChainIndexer(db, icedb.NewTable(db, "bltIndex-"), backend, size, 0, time.Millisecond*100, "bloomtrie", true)
}
