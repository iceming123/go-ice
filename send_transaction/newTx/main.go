...
func send(ip string) {
	...
	err = client.Call(&account, "ice_accounts")
	if err != nil {
		fmt.Println("ice_accounts Error", err.Error())
		return
	}
	if len(account) == 0 {
	...
	if txType {
		getBalance(client, 3)
		fmt.Println("go into sendIceRawTransaction")
		fmt.Println("========tx1 start==========")
		sendIceRawTransaction(client, account[0], account[1], "", "0xde0b6b3a7640000", "0x0")
		getBalance(client, 3)
		fmt.Println("========tx2 start==========")
		sendIceRawTransaction(client, account[0], account[2], account[1], "0xde0b6b3a7640000", "0x0")
		getBalance(client, 3)
		fmt.Println("========tx3 start==========")
		sendIceRawTransaction(client, account[0], account[2], account[1], "0xde0b6b3a7640000", "0xde0b6b3a7640000")
		getBalance(client, 3)
		fmt.Println("========tx4 start==========")
		sendIceRawTransaction(client, account[0], account[2], "", "0xde0b6b3a7640000", "0xde0b6b3a7640000")
		getBalance(client, 3)
		return
	}
...
func getBalance(client *rpc.Client, index int) {
	...
	var result string
	for i := 0; i < index; i++ {
		err := client.Call(&result, "ice_getBalance", account[i], "latest")
		if err != nil {
...
func sendIceRawTransaction(client *rpc.Client, from string, to string, payment string, value string, fee string) (string, error) {
	...
	var result string
	err = client.Call(&result, "ice_sendTransaction", mapData)
	if err != nil {
		log.Error("ice_sendTransaction", "err", err)
	}
	return result, err
}
...
func getBalanceValue(hex string) *big.Int {
	...
	//fmt.Println("ice_getBalance Ok:", balance.Div(balance, big.NewInt(ether)), " value ", value, " hex ", hex)
	return value
}
