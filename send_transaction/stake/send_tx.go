...
import (
	...
	tlog "github.com/iceming123/go-ice/log"
...
func loop(conn *iceclient.Client, header *types.Header, ctx *cli.Context) {
	loopMutex.Lock()
...
func startDelegateTx(conn *iceclient.Client, diff, number uint64, query bool) {
	for i := 0; i < len(delegateAddr); i++ {
		addr := delegateAddr[i]
		key := delegateKey[i]
...
func redistributionDelegate(conn *iceclient.Client) {
	loopCount = 0
...
func deposit(ctx *cli.Context, conn *iceclient.Client, value *big.Int) {
...
func getBalance(conn *iceclient.Client, address common.Address) *big.Int {
...
func sendOtherContractTransaction(client *iceclient.Client, f, toAddress common.Address, value *big.Int, privateKey *ecdsa.PrivateKey, input []byte, method string) (common.Hash, error) {
	blockMutex.Lock()
...
func sendContractTransaction(client *iceclient.Client, from, toAddress common.Address,
	...
	if types.StakingAddress == toAddress {
		if strings.Contains("withdrawImpawn", method) {
			...
			msg := icechain.CallMsg{From: from, To: &toAddress, GasPrice: gasPrice, Value: value, Data: input}
...
func getResult(conn *iceclient.Client, txHash common.Hash, contract bool, delegate bool) {
...
func querySendTx(conn *iceclient.Client, number uint64) {
...
func queryTx(conn *iceclient.Client, txHash common.Hash, contract bool, pending bool, delegate bool) bool {
	if pending {
		_, isPending, err := conn.TransactionByHash(context.Background(), txHash)
		if err != nil {
...
func PrintBalance(conn *iceclient.Client, from common.Address) {
	...
	fmt.Println("Your wallet balance is ", trueValue, "'true ", " current Total Stake ", types.ToCoin(sbalance))
}
...
func dialConn(ctx *cli.Context) (*iceclient.Client, string) {
	...
	conn, err := iceclient.Dial(url)
	if err != nil {
		log.Fatalf("Failed to connect to the Icechain client: %v", err)
	}
	return conn, url
}

func getPubKey(ctx *cli.Context, conn *iceclient.Client) (string, []byte, error) {
	var (
		pubkey string
		err    error
	)
...
func printBaseInfo(conn *iceclient.Client, url string) *types.Header {
	header, err := conn.HeaderByNumber(context.Background(), nil)
	if err != nil {
		log.Fatal(err)
	}
...
func queryStakingInfo(conn *iceclient.Client, query bool, delegate bool) (uint64, uint64) {
	header, err := conn.HeaderByNumber(context.Background(), nil)
	if err != nil {
		log.Fatal(err)
	}
	...
	msg := icechain.CallMsg{From: from, To: &types.StakingAddress, Data: input}
...
func queryDelegateInfo(conn *iceclient.Client, daAddress common.Address) (uint64, uint64) {
	header, err := conn.HeaderByNumber(context.Background(), nil)
	if err != nil {
		log.Fatal(err)
	}
	input := packInput("getDelegate", daAddress, from)
	msg := icechain.CallMsg{From: daAddress, To: &types.StakingAddress, Data: input}
	output, err := conn.CallContract(context.Background(), msg, header.Number)
	if err != nil {
		printError("method CallContract error", err)
	}
...
func queryRewardInfo(conn *iceclient.Client, snailNumber uint64, address common.Address) (bool, uint64) {
