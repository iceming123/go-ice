...
import (
	...
	ethereum "github.com/iceming123/go-ice"
...
func cancel(conn *iceclient.Client, value *big.Int) error {
...
func withdrawImpawn(conn *iceclient.Client, value *big.Int) (common.Hash, error) {
...
func delegateImpawn(conn *iceclient.Client, value *big.Int, address common.Address, key *ecdsa.PrivateKey) (common.Hash, error) {
...
func cancelDImpawn(conn *iceclient.Client, value *big.Int, address common.Address, key *ecdsa.PrivateKey) error {
...
func withdrawDImpawn(conn *iceclient.Client, value *big.Int, address common.Address, key *ecdsa.PrivateKey) (common.Hash, error) {
...
func filterLogs(client *iceclient.Client) {
	method := "Deposit"
	...
	for i := uint64(4706111); i < header.Number.Uint64(); {
		...
		for _, vLog := range logs {
			...
			if method == "SetFee" {
				...
				fmt.Println("value", types.ToCoin(deposit.Value), "fee ", deposit.Fee, "pk", hex.EncodeToString(deposit.Pubkey)) // foo
			}
...
func queryRewardImpawn(ctx *cli.Context) error {
	...
	err = client.Call(&head, "ice_getSnailBlockByNumber", "latest", false)
	if err != nil {
		fmt.Println("ice_getSnailBlockByNumber:", err.Error())
		return err
	}
	...
	for i := start; i < head.Number.ToInt().Uint64()-14; i++ {
		...
		err = client.Call(&reward, "ice_getChainRewardContent", hexutil.EncodeBig(new(big.Int).SetUint64(i)), address)
		if err != nil {
			fmt.Println("ice_getChainRewardContent:", err.Error())
			return err
		}
