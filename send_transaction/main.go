...
func getAccountBalance(client *rpc.Client, account string) *big.Int {
	...
	// get balance
	err := client.Call(&result, "ice_getBalance", account, "latest")
	if err != nil {
		fmt.Println("ice_getBalance Error:", err)
		msg <- false
		return nil
	}
...
func checkSonAccountBalance(client *rpc.Client, count int, main *big.Int) bool {
	...
	for i, v := range account {
		...
		fmt.Println("ice_getBalance son address ", account[i], " result ", balance, " i ", i, " true ", balanceTrue.Div(balanceTrue, big.NewInt(1000000000000000000)))
