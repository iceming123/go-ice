...
start:
	...
	if err != nil {
		fmt.Println(getTime(), "ice_accounts Error", err.Error())
		return
	}
	if len(account) == 0 {
	...
	// get balance
	var result string = ""
	err = client.Call(&result, "ice_getBalance", account[from], "latest")
	if err != nil {
		fmt.Println(getTime(), "ice_getBalance Error:", err)
		return
	} else {

		bl, _ := new(big.Int).SetString(result, 10)
		fmt.Println(getTime(), "ice_getBalance Ok:", bl, result)
	}
...
func sendTransaction(client *rpc.Client, account []string, wait *sync.WaitGroup) {
	...
	var result string
	client.Call(&result, "ice_sendTransaction", map_data)
	if result != "" {
		Count += 1
	}
}
