...
func newWorker(config *params.ChainConfig, engine consensus.Engine, coinbase common.Address, ice Backend, mux *event.TypeMux) *worker {
	worker := &worker{
		config:            config,
		engine:            engine,
		ice:               ice,
		...
		chain:             ice.SnailBlockChain(),
		fastchain:         ice.BlockChain(),
		proc:              ice.SnailBlockChain().Validator(),
		possibleUncles:    make(map[common.Hash]*types.SnailBlock),
		coinbase:          coinbase,
		agents:            make(map[Agent]struct{}),
		unconfirmed:       newUnconfirmedBlocks(ice.SnailBlockChain(), miningLogAtDepth),
		fastBlockNumber:   big.NewInt(0),
		atCommintNewWoker: false,
		fruitPoolMap:      make(map[uint64]*types.SnailBlock),
	}
	// Subscribe events for blockchain
	worker.chainHeadSub = ice.SnailBlockChain().SubscribeChainHeadEvent(worker.chainHeadCh)
	worker.chainSideSub = ice.SnailBlockChain().SubscribeChainSideEvent(worker.chainSideCh)
	worker.minedfruitSub = ice.SnailBlockChain().SubscribeNewFruitEvent(worker.minedfruitCh)

	worker.fruitSub = ice.SnailPool().SubscribeNewFruitEvent(worker.fruitCh)
		...
		for result := range w.recv {
			...
			if block.IsFruit() {
				...
				if w.minedFruit == nil {
					...
					w.ice.SnailPool().AddRemoteFruits(newFruits, true)
...
func (w *worker) commitNewWork() {
	...
	fruits := w.ice.SnailPool().PendingFruits()
		...
		for _, fruit := range fruits {
			...
			if rst := currentFastNumber.Cmp(fruit.FastNumber()); rst > 0 {
				...
				if err == nil {
					...
					w.ice.SnailPool().RemovePendingFruitByFastHash(fruit.FastHash())
