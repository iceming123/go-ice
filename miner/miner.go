...
func New(icechain Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine,
	...
	miner := &Miner{
		...
		worker:            newWorker(config, engine, common.Address{}, icechain, mux),
	...
	if !remoteMining {
		miner.Register(NewCPUAgent(icechain.SnailBlockChain(), engine))
		if !miner.singleNode {
