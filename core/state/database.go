...
type Trie interface {
	...
	Prove(key []byte, fromLevel uint, proofDb icedb.Putter) error
}
...
func NewDatabase(db icedb.Database) Database {
...
func NewDatabaseWithCache(db icedb.Database, cache int) Database {
...
func (m cachedTrie) Prove(key []byte, fromLevel uint, proofDb icedb.Putter) error {
