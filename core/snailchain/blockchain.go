...
// Package snailchain implements the icechain snailchain protocol.
package snailchain
...
type SnailBlockChain struct {
	...
	db          icedb.Database      // Low level persistent database to store final content in
...
// Processor.
func NewSnailBlockChain(db icedb.Database, chainConfig *params.ChainConfig, engine consensus.Engine, blockchain *core.BlockChain) (*SnailBlockChain, error) {
...
func (bc *SnailBlockChain) GetDatabase() icedb.Database {
	return bc.db
}
