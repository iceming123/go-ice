...
import (
	...
	rawdb "github.com/iceming123/go-ice/core/snailchain/rawdb"
...
type ChainIndexer struct {
	chainDb  icedb.Database      // Chain database to index the data from
	indexDb  icedb.Database      // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb, indexDb icedb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
