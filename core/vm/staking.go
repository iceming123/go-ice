...
// RunStaking execute icechain staking contract
...
func deposit(evm *EVM, contract *Contract, input []byte) (ret []byte, err error) {
	...
	if evm.StateDB.GetUnlockedBalance(from).Cmp(args.Value) < 0 {
		log.Error("Staking balance insufficient", "address", contract.caller.Address().String(), "value", args.Value)
	...
	if err != nil {
		log.Error("Staking deposit", "address", contract.caller.Address().String(), "value", args.Value, "error", err)
		return nil, err
	}
...
func depositAppend(evm *EVM, contract *Contract, input []byte) (ret []byte, err error) {
	...
	if evm.StateDB.GetUnlockedBalance(from).Cmp(amount) < 0 {
		log.Error("Staking balance insufficient", "address", contract.caller.Address().String(), "value", amount)
		return nil, ErrStakingInsufficientBalance
	}

	log.Info("Staking deposit extra", "number", evm.Context.BlockNumber.Uint64(), "address", contract.caller.Address().String(), "value", amount)
	impawn := NewImpawnImpl()
	err = impawn.Load(evm.StateDB, types.StakingAddress)
	if err != nil {
		log.Error("Staking load error", "error", err)
	...
	if err != nil {
		log.Error("Staking deposit extra", "address", contract.caller.Address().String(), "value", amount, "error", err)
		return nil, err
	}
...
func setFeeRate(evm *EVM, contract *Contract, input []byte) (ret []byte, err error) {
	...
	log.Info("Staking set fee", "number", evm.Context.BlockNumber.Uint64(), "address", contract.caller.Address().String(), "fee", fee)
	impawn := NewImpawnImpl()
	err = impawn.Load(evm.StateDB, types.StakingAddress)
	if err != nil {
		log.Error("Staking load error", "error", err)
	...
	if err != nil {
		log.Error("Staking fee", "address", contract.caller.Address().String(), "error", err)
		return nil, err
	}
...
func setPubkey(evm *EVM, contract *Contract, input []byte) (ret []byte, err error) {
	...
	log.Info("Staking set pubkey", "number", evm.Context.BlockNumber.Uint64(), "address", contract.caller.Address().String(), "pk", pubkey)
	impawn := NewImpawnImpl()
	err = impawn.Load(evm.StateDB, types.StakingAddress)
	if err != nil {
		log.Error("Staking load error", "error", err)
	...
	if err != nil {
		log.Error("Staking pubkey", "address", contract.caller.Address().String(), "error", err)
		return nil, err
	}
...
func delegate(evm *EVM, contract *Contract, input []byte) (ret []byte, err error) {
	...
	if err != nil {
		log.Error("Staking delegate", "address", contract.caller.Address().String(), "value", args.Value, "error", err)
		return nil, err
	}
	t3 := time.Now()
	err = impawn.Save(evm.StateDB, types.StakingAddress)
...
func undelegate(evm *EVM, contract *Contract, input []byte) (ret []byte, err error) {
	...
	log.Info("Staking undelegate", "number", evm.Context.BlockNumber.Uint64(), "address", contract.caller.Address().String(), "holder", args.Holder, "value", args.Value)
	impawn := NewImpawnImpl()
	err = impawn.Load(evm.StateDB, types.StakingAddress)
	if err != nil {
		log.Error("Staking load error", "error", err)
	...
	if err != nil {
		log.Error("Staking undelegate", "address", contract.caller.Address().String(), "value", args.Value, "error", err)
		return nil, err
	}
...
func cancel(evm *EVM, contract *Contract, input []byte) (ret []byte, err error) {
	...
	log.Info("Staking cancel", "number", evm.Context.BlockNumber.Uint64(), "address", contract.caller.Address().String(), "value", amount)
	impawn := NewImpawnImpl()
	err = impawn.Load(evm.StateDB, types.StakingAddress)
	if err != nil {
		log.Error("Staking load error", "error", err)
...
func withdraw(evm *EVM, contract *Contract, input []byte) (ret []byte, err error) {
	...
	log.Info("Staking withdraw", "number", evm.Context.BlockNumber.Uint64(), "address", contract.caller.Address().String(), "value", amount)
...
func withdrawDelegate(evm *EVM, contract *Contract, input []byte) (ret []byte, err error) {
	...
	log.Info("Staking withdraw", "number", evm.Context.BlockNumber.Uint64(), "address", contract.caller.Address().String(), "value", args.Value)
