...
import (
	...
	"github.com/iceming123/go-ice/core/asm"
...
func TestCall(t *testing.T) {
	state, _ := state.New(common.Hash{}, state.NewDatabase(icedb.NewMemDatabase()))
...
func benchmarkNonModifyingCode(gas uint64, code []byte, name string, b *testing.B) {
	cfg := new(Config)
	setDefaults(cfg)
	cfg.State, _ = state.New(common.Hash{}, state.NewDatabase(icedb.NewMemDatabase()))
	cfg.GasLimit = gas
	var (
