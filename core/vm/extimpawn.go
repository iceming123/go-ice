...
// EncodeRLP serializes b into the icechain RLP ImpawnImpl format.
...
func (i *ImpawnImpl) GetAllStakingAccountRPC(height uint64) map[string]interface{} {
	...
	for index, sa := range sas {
		...
		attr["staking"] = weitoCoin(sa.getAllStaking(height))
		attr["validStaking"] = weitoCoin(sa.getValidStaking(height))
...
func (i *ImpawnImpl) GetAllCancelableAssetRPC(addr common.Address) []CancelableAsset {
	...
	for key, value := range assets {
		attr := CancelableAsset{Value: weitoCoin(value), Address: key}
		attrs = append(attrs, attr)
	}
	return attrs
}
...
func daSDisplay(das []*DelegationAccount, height uint64) []map[string]interface{} {
	...
	for _, da := range das {
		attr := make(map[string]interface{})
		attr["saAddress"] = da.SaAddress.String()
		attr["delegate"] = weitoCoin(da.getAllStaking(height))
		attr["validDelegate"] = weitoCoin(da.getValidStaking(height))
...
func unitDisplay(uint *impawnUnit) map[string]interface{} {
	attr := make(map[string]interface{})
	attr["address"] = uint.Address.String()
...
func pvSDisplay(pvs []*PairstakingValue) []map[string]interface{} {
	...
	for _, pv := range pvs {
		attr := make(map[string]interface{})
		attr["amount"] = weitoCoin(pv.Amount)
...
func riSDisplay(ris []*RedeemItem) []map[string]interface{} {
	...
	for _, ri := range ris {
		attr := make(map[string]interface{})
		attr["amount"] = weitoCoin(ri.Amount)
...
func weitoCoin(val *big.Int) string {
