...
func FetchOneToWei(sas []*SARewardInfos, addr common.Address) map[string]interface{} {
	items := make([]*RewardInfo, 0, 0)
	for _, val := range sas {
		if len(val.Items) > 0 {
			saAddr := val.getSaAddress()
...
func (s *SARewardInfos) StringToWei() map[string]interface{} {
...
func (s *ChainReward) CommitteeRewardInfo() map[string]interface{} {
	...
	for _, v := range s.CommitteeBase {
		infos = append(infos, v.StringToWei())
	}
	feild := map[string]interface{}{
		"committeeReward": infos,
	}
