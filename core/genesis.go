...
import (
	...
	snaildb "github.com/iceming123/go-ice/core/snailchain/rawdb"
...
func SetupGenesisBlock(db icedb.Database, genesis *Genesis) (*params.ChainConfig, common.Hash, common.Hash, error) {
...
func SetupGenesisBlockForLes(db icedb.Database) (*params.ChainConfig, common.Hash, error) {
...
//
// The returned chain configuration is never nil.
func setupFastGenesisBlock(db icedb.Database, genesis *Genesis) (*params.ChainConfig, common.Hash, error) {
	if genesis != nil && genesis.Config == nil {
...
func setupFastGenesisBlockForLes(db icedb.Database) (*params.ChainConfig, common.Hash, error) {
	// Just commit the new block if there is no stored genesis block.
...
func (g *Genesis) CommitFast(db icedb.Database) (*types.Block, error) {
...
func (g *Genesis) ToFastBlock(db icedb.Database) *types.Block {
	if db == nil {
		db = icedb.NewMemDatabase()
	}
...
// The block is committed as the canonical head block.
func (g *Genesis) MustFastCommit(db icedb.Database) *types.Block {
...
//
// The returned chain configuration is never nil.
func setupSnailGenesisBlock(db icedb.Database, genesis *Genesis) (*params.ChainConfig, common.Hash, error) {
	if genesis != nil && genesis.Config == nil {
		return params.AllMinervaProtocolChanges, common.Hash{}, errGenesisNoConfig
	}
	// Just commit the new block if there is no stored genesis block.
...
// to the given database (or discards it if nil).
func (g *Genesis) ToSnailBlock(db icedb.Database) *types.SnailBlock {
	if db == nil {
		db = icedb.NewMemDatabase()
	}
...
// The block is committed as the canonical head block.
func (g *Genesis) CommitSnail(db icedb.Database) (*types.SnailBlock, error) {
...
// The block is committed as the canonical head block.
func (g *Genesis) MustSnailCommit(db icedb.Database) *types.SnailBlock {
...
// DefaultGenesisBlock returns the Icechain main net snail block.
...
func GenesisFastBlockForTesting(db icedb.Database, addr common.Address, balance *big.Int) *types.Block {
...
func GenesisSnailBlockForTesting(db icedb.Database, addr common.Address, balance *big.Int) *types.SnailBlock {
	g := Genesis{Alloc: types.GenesisAlloc{addr: {Balance: balance}}, Config: params.AllMinervaProtocolChanges}
...
func (g *LesGenesis) CommitFast(db icedb.Database) (*types.Block, error) {
