...
// Package icestats implements the network stats reporting service.
package icestats
...
import (
	...
	"github.com/iceming123/go-ice/common/mclock"
...
// Service implements an Icechain netstats reporting daemon that pushes local
// chain statistics up to a monitoring server.
type Service struct {
	server *p2p.Server      // Peer-to-peer server to retrieve networking infos
	ice    *ice.Icechain    // Full Icechain service if monitoring a full node
	les    *les.LightICE    // Light Icechain service if monitoring a light node
...
func New(url string, ethServ *ice.Icechain, lesServ *les.LightICE) (*Service, error) {
...
func (s *Service) loop() {
	...
	if s.ice != nil {
		blockchain = s.ice.BlockChain()
		txpool = s.ice.TxPool()
		snailBlockChain = s.ice.SnailBlockChain()
	} else {
...
func handleHistCh(msg map[string][]interface{}, s *Service, command string) string {
	...
	if !ok {
		...
		return "continue" // Icestats sometime sends invalid history requests, ignore those
	}
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["ice"]; info != nil {
		network = fmt.Sprintf("%d", info.(*ice.NodeInfo).Network)
		protocol = fmt.Sprintf("ice/%d", ice.ProtocolVersions[0])
	} else {
...
func (s *Service) reportLatency(conn *websocket.Conn) error {
	// Send the current time to the icestats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to icestats", "latency", latency)
...
func (s *Service) reportBlock(conn *websocket.Conn, block *types.Block) error {
	...
	log.Trace("Sending new block to icestats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportSnailBlock(conn *websocket.Conn, block *types.SnailBlock) error {
	...
	// Assemble the block report and send it to the server
	log.Trace("Sending new snailBlock to icestats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) assembleBlockStats(block *types.Block) *blockStats {
	...
	if s.ice != nil {
		...
		if block == nil {
			block = s.ice.BlockChain().CurrentBlock()
		}
		header = block.Header()
...
func (s *Service) assembleSnailBlockStats(block *types.SnailBlock) *snailBlockStats {
	...
	// Full nodes have all needed information available
	if block == nil {
		block = s.ice.SnailBlockChain().CurrentBlock()
	}
	header = block.Header()
	td = s.ice.SnailBlockChain().GetTd(header.Hash(), header.Number.Uint64())
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(list) > 0 {
		...
		var head int64
		if s.ice != nil {
			head = s.ice.BlockChain().CurrentHeader().Number.Int64()
		} else {
	...
	for i, number := range indexes {
		...
		if s.ice != nil {
			block = s.ice.BlockChain().GetBlockByNumber(number)
		} else {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to icestats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportSnailHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(list) > 0 {
		...
		var head int64
		if s.ice != nil {
			head = s.ice.SnailBlockChain().CurrentHeader().Number.Int64()
			start := head - historyUpdateRange + 1
			if start < 0 {
				start = 0
			}
	...
	for i, number := range indexes {
		...
		if s.ice != nil {
			snailBlock = s.ice.SnailBlockChain().GetBlockByNumber(number)
			// If we do have the block, add to the history and continue
			if snailBlock != nil {
	...
	// Assemble the history report and send it to the server
	if len(history) > 0 {
		log.Trace("Sending historical snaiBlocks to icestats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
		log.Trace("No history to send to stats server")
	}
	stats := map[string]interface{}{
...
func (s *Service) reportPending(conn *websocket.Conn) error {
	...
	var pending int
	if s.ice != nil {
		pending, _ = s.ice.TxPool().Stats()
	} else {
		pending = s.les.TxPool().Stats()
	}
	// Assemble the transaction stats and send it to the server
	log.Trace("Sending pending transactions to icestats", "count", pending)
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	if s.ice != nil {
		mining = s.ice.Miner().Mining()
		hashrate = int(s.ice.Miner().HashRate())

		sync := s.ice.Downloader().Progress()
		syncing = s.ice.BlockChain().CurrentHeader().Number.Uint64() >= sync.HighestFastBlock

		price, _ := s.ice.APIBackend.SuggestPrice(context.Background())
		gasprice = int(price.Uint64())

		isCommitteeMember = s.ice.PbftAgent().IsCommitteeMember()
		isLeader = s.ice.PbftAgent().IsLeader()
	} else {
		sync := s.les.Downloader().Progress()
		syncing = s.les.BlockChain().CurrentHeader().Number.Uint64() >= sync.HighestFastBlock
	}
	// Assemble the node stats and send it to the server
	log.Trace("Sending node details to icestats")
	nodeStats := &nodeStats{
		Active:            true,
		Mining:            mining,
		Hashrate:          hashrate,
