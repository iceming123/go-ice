...
func (n *Node) OpenDatabase(name string, cache, handles int) (icedb.Database, error) {
	if n.config.DataDir == "" {
		return icedb.NewMemDatabase(), nil
	}
	return icedb.NewLDBDatabase(n.config.ResolvePath(name), cache, handles)
}
