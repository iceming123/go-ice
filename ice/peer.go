...
// PeerInfo represents a short summary of the Icechain sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // Icechain protocol version negotiated
...
// Handshake executes the ice protocol handshake, negotiating version number,
...
func (p *peer) String() string {
	return fmt.Sprintf("Peer %s [%s]", p.id,
		fmt.Sprintf("ice/%2d", p.version),
	)
}

// peerSet represents the collection of active peers currently participating in
// the Icechain sub-protocol.
type peerSet struct {
