...
type stateReq struct {
	...
	peer     ice.PeerConnection         // Peer that we're requesting from
...
func (d *Downloader) SyncState(root common.Hash) *stateSync {
	...
	case <-d.quitCh:
		s.err = ice.ErrCancelStateFetch
		close(s.done)
	}
	return s
}

func (d *Downloader) SyncStateFd(root common.Hash) ice.StateSyncInter {
	s := newStateSync(d, root)
	select {
	case d.stateSyncStart <- s:
	case <-d.quitCh:
...
func (d *Downloader) runStateSync(s *stateSync) *stateSync {
	...
	peerDrop := make(chan ice.PeerConnection, 1024)
...
func (s *stateSync) loop() (err error) {
	...
	newPeer := make(chan ice.PeerConnection, 1024)
	...
	for s.sched.Pending() > 0 {
		...
		case <-s.cancel:
			return ice.ErrCancelStateFetch
...
func (s *stateSync) commit(force bool) error {
	if !force && s.bytesUncommitted < icedb.IdealBatchSize {
		return nil
	}
	start := time.Now()
