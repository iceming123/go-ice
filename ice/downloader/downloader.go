...
import (
	...
	ice "github.com/iceming123/go-ice/ice/types"
	...
	//"github.com/iceming123/go-ice/trie"
)
...
type Downloader struct {
	...
	peers      *ice.PeerSet // Set of active peers from which download can proceed

	stateDB icedb.Database
	...
	// Callbacks
	dropPeer ice.PeerDropFn // Drops a peer for misbehaving
	...
	// Channels
	headerCh     chan ice.DataPack         // [eth/62] Channel receiving inbound block headers
	bodyCh       chan ice.DataPack         // [eth/62] Channel receiving inbound block bodies
	...
	stateCh        chan ice.DataPack // [eth/63] Channel receiving inbound node state data
	...
	chainInsertHook func([]*ice.FetchResult)   // Method to call upon inserting a chain of blocks (possibly in multiple invocations)
...
func New(mode SyncMode, checkpoint uint64, stateDb icedb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer ice.PeerDropFn, fdown *fastdownloader.Downloader) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	...
	dl := &Downloader{
		...
		peers:          ice.NewPeerSet(),
		rttEstimate:    uint64(rttMaxEstimate),
		rttConfidence:  uint64(1000000),
		blockchain:     chain,
		lightchain:     lightchain,
		dropPeer:       dropPeer,
		headerCh:       make(chan ice.DataPack, 1),
		bodyCh:         make(chan ice.DataPack, 1),
		bodyWakeCh:     make(chan bool, 1),
		headerProcCh:   make(chan []*types.SnailHeader, 1),
		quitCh:         make(chan struct{}),
		fastDown:       fdown,
		stateCh:        make(chan ice.DataPack),
...
func (d *Downloader) RegisterPeer(id string, version int, ip string, peer ice.Peer) error {
...
func (d *Downloader) RegisterLightPeer(id string, version int, ip string, peer ice.LightPeer) error {
...
func (d *Downloader) synchronise(id string, hash common.Hash, td *big.Int, mode SyncMode) error {
	...
	for _, ch := range []chan ice.DataPack{d.headerCh, d.bodyCh} {
...
func (d *Downloader) syncWithPeer(p ice.PeerConnection, hash common.Hash, td *big.Int) (err error) {
	if p.GetVersion() < 62 {
		return errTooOld
	}

	log.Debug("Snail Synchronising with the network", "peer", p.GetID(), "ice", p.GetVersion(), "head", hash, "td", td, "mode", d.mode)
...
func (d *Downloader) fetchHeight(p ice.PeerConnection) (*types.SnailHeader, error) {
...
func (d *Downloader) findAncestor(p ice.PeerConnection, remoteHeader *types.SnailHeader) (uint64, error) {
...
func (d *Downloader) fetchHeaders(p ice.PeerConnection, from uint64, pivot uint64) error {
...
func (d *Downloader) fillHeaderSkeleton(from uint64, skeleton []*types.SnailHeader) ([]*types.SnailHeader, int, error) {
	...
	var (
		deliver = func(packet ice.DataPack) (int, error) {
			pack := packet.(*headerPack)
			return d.queue.DeliverHeaders(pack.peerID, pack.headers, d.headerProcCh)
		}
		expire   = func() map[string]int { return d.queue.ExpireHeaders(d.requestTTL()) }
		throttle = func() bool { return false }
		reserve  = func(p ice.PeerConnection, count int) (*ice.FetchRequest, bool, error) {
			return d.queue.ReserveHeaders(p, count), false, nil
		}
		fetch = func(p ice.PeerConnection, req *ice.FetchRequest) error {
			return p.FetchHeaders(req.From, MaxHeaderFetch)
		}
		capacity = func(p ice.PeerConnection) int { return p.HeaderCapacity(d.requestRTT()) }
		setIdle  = func(p ice.PeerConnection, accepted int) { p.SetHeadersIdle(accepted) }
	)
...
func (d *Downloader) fetchBodies(from uint64) error {
	...
	var (
		...
		fetch    = func(p ice.PeerConnection, req *ice.FetchRequest) error { return p.FetchBodies(req) }
		capacity = func(p ice.PeerConnection) int { return p.BlockCapacity(d.requestRTT()) }
		setIdle  = func(p ice.PeerConnection, accepted int) { p.SetBodiesIdle(accepted) }
	)
...
func (d *Downloader) fetchParts(errCancel error, deliveryCh chan ice.DataPack, deliver func(ice.DataPack) (int, error), wakeCh chan bool,
	expire func() map[string]int, pending func() int, inFlight func() bool, throttle func() bool, reserve func(ice.PeerConnection, int) (*ice.FetchRequest, bool, error),
	fetchHook func([]*types.SnailHeader), fetch func(ice.PeerConnection, *ice.FetchRequest) error, cancel func(*ice.FetchRequest), capacity func(ice.PeerConnection) int,
	idle func() ([]ice.PeerConnection, int), setIdle func(ice.PeerConnection, int), kind string) error {
...
func (d *Downloader) processFullSyncContent(p ice.PeerConnection, hash common.Hash, td *big.Int, remoteHeader *types.SnailHeader) error {
	...
	if d.mode == FastSync || d.mode == SnapShotSync {
		...
		go func() {
			if err := stateSync.Wait(); err != nil && err != ice.ErrCancelStateFetch {
...
func (d *Downloader) importBlockResults(results []*ice.FetchResult, p ice.PeerConnection, hash common.Hash, td *big.Int, remoteHeader *types.SnailHeader) error {
...
func (d *Downloader) importBlockAndSyncFast(blocks []*types.SnailBlock, p ice.PeerConnection, hash common.Hash) error {
	firstB := blocks[0]
...
func (d *Downloader) deliver(id string, destCh chan ice.DataPack, packet ice.DataPack, inMeter, dropMeter metrics.Meter) (err error) {
