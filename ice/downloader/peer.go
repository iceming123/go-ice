...
func newPeerConnection(id string, version int, peer ice.Peer, logger log.Logger) *peerConnection {
...
func (p *peerConnection) GetPeer() ice.Peer      { return p.peer }
func (p *peerConnection) SetPeer(peer ice.Peer)  { p.peer = peer }
...
func (p *peerConnection) FetchReceipts(request *ice.FetchRequest) error {
	// Sanity check the protocol version
	if p.version < 63 {
...
func (p *peerConnection) FetchBodies(request *ice.FetchRequest) error {
	// Sanity check the protocol version
	if p.version < 62 {
