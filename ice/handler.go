...
import (
	...
	"github.com/iceming123/go-ice/ice/fetcher"
	snailfetcher "github.com/iceming123/go-ice/ice/fetcher/snail"
...
// NewProtocolManager returns a new Icechain sub protocol manager. The Icechain sub protocol manages peers capable
// with the Icechain network.
func NewProtocolManager(config *params.ChainConfig, mode downloader.SyncMode, networkID uint64, mux *event.TypeMux, txpool txPool, SnailPool SnailPool, engine consensus.Engine, blockchain *core.BlockChain, snailchain *snailchain.SnailBlockChain, chaindb icedb.Database, agent *PbftAgent) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) removePeer(id string, call uint32) {
	...
	log.Debug("Removing Icechain peer", "peer", id, "recipients", len(pm.peers.peers))

	// Unregister the peer from the downloader and Icechain peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping ICEChain protocol")
	...
	log.Info("Icechain protocol stopped")
}
...
func resolveVersionFromName(name string) bool {
	str := name
	flag := "Gice/v0.8.2"
	if !strings.Contains(str, "Gice/v0.8") {
		return true
	}
...
// handle is the callback invoked to manage the life cycle of an ice peer. When
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Icechain peer connected", "name", p.Name(), "remoteAddr", p.RemoteAddr())

	// Execute the Icechain handshake
	var (
	...
	if p.version >= ice64 {
		if err := p.SnapHandshake(pm.networkID, td, hash, genesis.Hash(), fastHash, fastHeight, gcHeight, commitHeight); err != nil {
			p.Log().Debug("Icechain handshake failed", "err", err)
			return err
		}
	} else {
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Icechain peer registration failed", "err", err)
		return err
	}
	...
	if err := pm.downloader.RegisterPeer(p.id, p.version, p.RemoteAddr().String(), p); err != nil {
		p.Log().Error("Icechain downloader registerPeer registration failed", "err", err)
		return err
	}

	if err := pm.fdownloader.RegisterPeer(p.id, p.version, p); err != nil {
		p.Log().Error("Icechain fdownloader registerPeer registration failed", "err", err)
		return err
	}
	...
	for {
		err := pm.handleMsg(p)
		if err != nil {
			p.Log().Info("Icechain message handling failed", "remoteAddr", p.RemoteAddr(), "err", err)
			return err
		}
	}
}
...
// NodeInfo represents a short summary of the Icechain sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network      uint64              `json:"network"`      // Icechain network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
