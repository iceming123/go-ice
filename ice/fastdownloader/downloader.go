...
import (
	...
	"github.com/iceming123/go-ice/consensus/tbft/help"
...
type Downloader struct {
	...
	receiptCh     chan ice.DataPack    // [eth/63] Channel receiving inbound receipts
...
func New(mode SyncMode, stateDb icedb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer ice.PeerDropFn) *Downloader {
	...
	dl := &Downloader{
		...
		receiptCh:     make(chan ice.DataPack, 1),
...
func (d *Downloader) synchronise(id string, hash common.Hash, mode SyncMode, origin uint64, height uint64) error {
	...
	for _, ch := range []chan ice.DataPack{d.headerCh, d.bodyCh, d.receiptCh} {
...
func (d *Downloader) syncWithPeer(p ice.PeerConnection, hash common.Hash, origin uint64, height uint64) (err error) {
...
func (d *Downloader) fetchHeaders(p ice.PeerConnection, from uint64, height int, pivot uint64) error {
...
func (d *Downloader) fetchReceipts(from uint64) error {
	...
	var (
		...
		fetch    = func(p ice.PeerConnection, req *ice.FetchRequest) error { return p.FetchReceipts(req) }
		capacity = func(p ice.PeerConnection) int { return p.ReceiptCapacity(d.requestRTT()) }
		setIdle  = func(p ice.PeerConnection, accepted int) { p.SetReceiptsIdle(accepted) }
	)
...
func (d *Downloader) fetchParts(errCancel error, deliveryCh chan ice.DataPack, deliver func(ice.DataPack) (int, error), wakeCh chan bool,
	...
	fetchHook func([]*types.Header), fetch func(ice.PeerConnection, *ice.FetchRequest) error, cancel func(*ice.FetchRequest), capacity func(ice.PeerConnection) int,
...
func (d *Downloader) importBlockResults(results []*ice.FetchResult) error {
...
func splitAroundPivot(pivot uint64, results []*ice.FetchResult) (p *ice.FetchResult, before, after []*ice.FetchResult) {
...
func (d *Downloader) commitPivotBlock(result *ice.FetchResult) error {
...
func (d *Downloader) commitFastSyncData(results []*ice.FetchResult) error {
	// Check for any early termination requests
	if len(results) == 0 {
		return nil
	}
...
func (d *Downloader) deliverOne(id string, destCh chan ice.DataPack, packet ice.DataPack, inMeter, dropMeter metrics.Meter) (err error) {
	// Update the delivery metrics for both good and failed deliveries
