...
func TestReward2(t *testing.T) {
	...
	for i := 0; i < 25; i++ {
		...
		fmt.Println("committeeAward:", cc, "minerAward:", mm,
			"minerFruitAward:", mf)
		fmt.Println("committeeAward:", toCoin(cc).Text('f', 6), "minerAward:", toCoin(mm).Text('f', 6),
			"minerFruitAward:", toCoin(mf).Text('f', 6),
			"all:", toCoin(new(big.Int).Add(mm, mf)).Text('f', 6))
	...
	fmt.Println("allReward", toCoin(allReward).Text('f', 10))
...
func TestRewardWithTimes01(t *testing.T) {
	...
	fmt.Println("init reward", toCoin(params.InitReward).Text('f', 6))
	...
	for i := int64(0); i < count; i++ {
		...
		if old.Cmp(value) != 0 {
			...
			fmt.Println("block height", i, "reward", toCoin(value).Text('f', 6))
		}
	}
}
