...
import (
	...
	"github.com/iceming123/go-ice/les/flowcontrol"
...
func getGenesisHash(ice *ice.Icechain) common.Hash {
	block := ice.BlockChain().GetBlockByNumber(params.LesProtocolGenesisBlock)
	hash := block.Hash()
	return hash
}

func NewLesServer(ice *ice.Icechain, config *ice.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	genesisHash := getGenesisHash(ice)
	pm, err := NewProtocolManager(ice.BlockChain().Config(), light.DefaultServerIndexerConfig, false,
		config.NetworkId, ice.EventMux(), ice.Engine(), newPeerSet(), ice.BlockChain(),
		ice.TxPool(), ice.ChainDb(), nil, nil, nil, quitSync, new(sync.WaitGroup), genesisHash)
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(ice.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		lesCommons: lesCommons{
			...
			chtIndexer:       light.NewChtIndexer(ice.ChainDb(), nil, params.CHTFrequencyServer, params.HelperTrieProcessConfirmations),
			bloomTrieIndexer: light.NewBloomTrieIndexer(ice.ChainDb(), nil, params.BloomBitsBlocks, params.BloomTrieFrequency),
			protocolManager:  pm,
		},
		quitSync:  quitSync,
		lesTopics: lesTopics,
	...
	srv.chtIndexer.Start(ice.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(ice.ChainDb())
	return srv, nil
}
...
func newCostStats(db icedb.Database) *requestCostStats {
